...
test.serial(
  ...
  t => {
    const { stderr } = run(
      ['generate', path.join(genPath, 'jsonic-collection.json')],
      { cwd: genPath, reject: false }
    )
    t.true(stderr.includes('Make sure that jsonic-collection.json exist'))
  }
)
...
test.serial(
  ...
  t => {
    ...
    t.true(
      fs.existsSync(
        path.join(docsDirPath, '.vuepress', 'components', 'JsonicRequest.vue')
      )
    )
  }
)
