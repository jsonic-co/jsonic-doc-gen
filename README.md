<div align="center">
  ...
  <p>
    An API documentation generator CLI by https://jsonic.ir
  </p>
</div>
...
`path` - path to `jsonic-collection.json` exported from [jsonic.ir](https://jsonic.ir/)
...
_Jsonic Doc Gen generates documentation (using Vuepress) based on Jsonic collections._

**1.** Navigate to https://jsonic.ir/doc and export a collection. (by clicking on the `Import/Export` button in the Collections pane).

**2.** Then generate documentation using the following command:

```
$ npx hdg generate jsonic-collection.json
```
