...
const generateAPIDoc = async (filePath, opts) => {
  ...
  if (!fs.existsSync(filePath)) {
    logger.error(
      `\n Make sure that jsonic-collection.json exist in ${process.cwd()}`
    )
    process.exit(1)
  }
  ...
  if (requestButtons) {
    // Create a global component at .vuepress/components/JsonicRequest.vue from src/templates/components/JsonicRequest.vue
    ...
    try {
      ...
      fs.copyFileSync(
        ...
        path.join(vuepressComponentsDirPath, 'JsonicRequest.vue')
      )
    } catch (e) {
      logger.error(e)
      process.exit(1)
  ...
  // Parse jsonic-collections.json config file
      ...
      Object.keys(request)
        ...
        .forEach(key => {
          try {
            ...
            logger.error(
              `\n Check key: ${key}. Make sure that jsonic-collection.json schema is valid`
            )
            process.exit(1)
          }
        })

      // Render JsonicRequest component
