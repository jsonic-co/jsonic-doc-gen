<template>
  <section class="jsonic-request">
    <div>
      <label :for="`jsonic-request__select-${_uid}`">Request code sample</label>
      <select
        :id="`jsonic-request__select-${_uid}`"
    ...
    <button
      :class="{ 'jsonic-request__fetching': api.fetching }"
    ...
    <div v-if="api.loaded" class="jsonic-request__response">
      <div class="jsonic-request__api-message">
        <h4>Response</h4>
        <div class="language-json" v-if="!api.fetching">
          <pre
            class="jsonic-request__language-json"
